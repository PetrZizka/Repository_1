...
if users.get(username) == password:
    print("access granted.", "Hi,", username, "Welcome to our application" )
...
if (text_number) == 1:
    clean_words = [word.strip(",´.'() ") for word in TEXTS[0].split() if len(word) >= 2]
    ...
    print("There are", len(title_case_word), " words in the selected text.")
    upper_case_word = {word : clean_words.count(word) for word in clean_words if word.isupper()}
    print("There are", len(upper_case_word), " words in the selected text.")
    lower_case_word = {word : clean_words.count(word) for word in clean_words if word.islower()}
    print("There are", len(lower_case_word), " words in the selected text.")
...
#konec textu1
    ...
    clean_words = [word.strip(",´.'() ") for word in TEXTS[1].split() if len(word) >= 2]
...
#konec druheho textu
    ...
    clean_words = [word.strip(",´.'() ") for word in TEXTS[2].split() if len(word) >= 2]
